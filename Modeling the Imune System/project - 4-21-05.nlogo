...
to setup-globals
  ...
  set initial-red-cell 2000
  set initial-white-cell initial-red-cell / 500;(initial-red-cell / 500) ;the number of white blood cells equals red-cells/500 from research
...
to go
  ...
  if show-red-cell-trails = true [ask red-cell [pen-down]]
  if show-red-cell-trails = false [ask red-cell [pen-up] reset [orange]]
  if show-white-cell-trails = true [ask white-cell [pen-down]]
  if show-white-cell-trails = false [ask white-cell [pen-up] reset [white]]
  if show-virus-trails = true [ask virus-1 [pen-down] ask virus-2 [pen-down] ask virus-3 [pendown] ask virus-4 [pendown]]
  if show-virus-trails = false [ask virus-1 [pen-up] ask virus-2 [pen-up] ask virus-3 [pen-up] ask virus-4 [pen-up] reset [green]]
end

to reset [turtle-color] ;this is used to reset all the patches to their true colors depending on their type after deleting pen trails done by turtles
  ask patches [if pcolor = turtle-color and skin? = true [set pcolor yellow] if pcolor = turtle-color and hole? = true [set pcolor black] if pcolor = turtle-color and inside? = true and skin? = false [set pcolor black]]
end
...
to go-white-cell
  ask white-cell [fd 1 
    if count other-virus-1-here >= 1 [ask virus-1-here[die] set confirmed-kills confirmed-kills + 1]
    if count other-virus-2-here >= 1 [ask virus-2-here[die] set confirmed-kills confirmed-kills + 1]
    if count other-virus-3-here >= 1 [ask virus-3-here[die] set confirmed-kills confirmed-kills + 1]
    if count other-virus-4-here >= 1 [ask virus-4-here[die] set confirmed-kills confirmed-kills + 1]
    if count other-white-cell-here >= 1 or count other-red-cell-here >= 1 [set heading (- heading)]
    if xcor >= skin-bound-right or xcor <= skin-bound-left[set heading (- heading)]
    ]
end
