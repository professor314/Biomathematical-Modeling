...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
to setup-virus
  create-custom-virus initial-virus [ set color green set strain random virus-strains ]
  ask virus [
    ;set shape " "
...
to setup-white-cell
  create-custom-white-cell initial-white-cell [set speed 1 set shape "whiteblood" set color white ifelse random 2 = 1 [set out-of-body? false set heading (360 - random 10)][set heading (360 + random 10)]] ;set headings to be mostly forward but having some small component in the x or -x direction]
    ask white-cell [if who < initial-white-cell [(set xcor random skin-bound-right + 1) (set ycor random screen-size-y)]
                    if who >= initial-white-cell [(set xcor (- random skin-bound-right - 1)) (set ycor random screen-size-y)]]
end

to setup-patches
  ask patches [set outside? true
  set skin? false
...
to setup-skin
  let n 0
  repeat (skin-thickness) [ask  patches [if (pxcor = (skin-bound-right - n) or pxcor = (skin-bound-left + n)) [set pcolor yellow set skin? true set inside? true set outside? false]] set n (n + 1)]
  if skin-pores? [ask patches [if (pcolor = yellow) and random (poressness) = 1 [set pcolor black set skin? false set hole? true]]]
end

to setup-red-cell
  create-custom-red-cell initial-red-cell [
    set speed 1 set shape "redblood" set color orange set out-of-body? false ifelse random 2 = 1 [
  ...
  ask red-cell [if who < initial-red-cell / 2 [(set xcor random skin-bound-right ) (set ycor random screen-size-y)]
                if (who >= initial-red-cell / 2) [(set xcor (- random skin-bound-right - 1 )) (set ycor (random screen-size-y - 1))]]
end
...
to go-virus
  ask virus [
    ...
    if hatch-count-down = 0 [hatch 3[set wandering? true] set wandering? true]
    ]
end

to go-red-cell
  ask red-cell [fd speed
...
to go-white-cell
  ask white-cell [fd speed 
    if count other-virus-here >= 1 [ask virus-here[die] set confirmed-kills confirmed-kills + 1]
  ...
  ask white-cell with [out-of-body? = true][set heading 45 fd 1]
end
...
to find-virus ;for white cells
 ifelse any? virus in-radius blood-sensing-radius [
   set heading (towards min-one-of (virus in-radius blood-sensing-radius)[distance myself ])][ifelse random 2 = 1 [set heading 360 - random 15][set heading 360 + random 15]]
end
