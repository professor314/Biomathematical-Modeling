...
to setup-patches
  ask patches [
    ...
    set oxygen? false
    set oxygen 0
    if pxcor >= skin-bound-left and pxcor <= skin-bound-right [set inside? true set outside? false]
  ]
  setup-skin
  setup-flesh
  setup-breath
...
to setup-flesh
  let n 0
  repeat (flesh-thickness) [ask  patches [if (pxcor = (skin-bound-right - skin-thickness - n) or pxcor = (skin-bound-left + skin-thickness + n)) [set pcolor 26 set flesh? true set inside? true set cells-here cells-per-patch set outside? false]] set n (n + 1)]
